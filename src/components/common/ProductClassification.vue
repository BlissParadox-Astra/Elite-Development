    ...
    data() {
        return {
            newItem: this.title === "Brand Module" ? (this.product ? this.product.brand : "") :
                   this.title === "Category Module" ? (this.product ? this.product.category : "") :
                   "",        };
    },
    methods: {
        saveItem() {
      if (this.title === "Category Module") {
        if (this.productIndex !== -1) {
          this.$emit("category-edited", this.newItem, this.productIndex);
        } else {
          this.$emit("category-added", this.newItem);
        }
